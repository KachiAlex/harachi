...
const batchSchema = Joi.object({
  ...
  supplierBatch: Joi.string().optional(),
  quantity: Joi.number().positive().required(),
  uomId: Joi.string().required(),
  qualityStatus: Joi.string().valid('PENDING', 'PASSED', 'FAILED', 'QUARANTINE').default('PENDING'),
  notes: Joi.string().optional()
});

const batchMovementSchema = Joi.object({
  batchId: Joi.string().required(),
  movementType: Joi.string().valid('IN', 'OUT', 'TRANSFER', 'ADJUSTMENT').required(),
  quantity: Joi.number().required(),
  fromBranchId: Joi.string().optional(),
  toBranchId: Joi.string().optional(),
  referenceType: Joi.string().valid('PURCHASE', 'SALE', 'PRODUCTION', 'TRANSFER', 'ADJUSTMENT').required(),
...
router.get('/batches', verifyToken, async (req, res) => {
  try {
    ...
    if (expiryDate) {
      const expiryDateObj = new Date(expiryDate as string);
...
router.get('/batches/:id/traceability', verifyToken, async (req, res) => {
  try {
    ...
    for (const movement of movements) {
      if ((movement as any).movementType === 'IN') {
        currentQuantity += (movement as any).quantity;
      } else if ((movement as any).movementType === 'OUT') {
        currentQuantity -= (movement as any).quantity;
      }
    }
