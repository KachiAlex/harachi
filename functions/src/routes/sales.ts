...
const updateSalesOrderSchema = Joi.object({
  status: Joi.string().valid('Draft', 'Confirmed', 'Shipped', 'Delivered', 'Cancelled').optional(),
  deliveryDate: Joi.date().optional(),
  items: Joi.array().items(Joi.object({
    itemId: Joi.string().required(),
    uomId: Joi.string().required(),
  ...
  try {
    ...
    if (search) {
      ...
      customers = customers.filter(customer => 
        (customer as any).name.toLowerCase().includes(searchTerm) ||
        (customer as any).code.toLowerCase().includes(searchTerm) ||
        ((customer as any).email && (customer as any).email.toLowerCase().includes(searchTerm))
      );
    }
