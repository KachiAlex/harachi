...
const SetupWizard: React.FC = () => {
  ...
  const loadSavedData = useCallback(async () => {
    try {
      ...
      // Resolve company by ID or by code
      let companyId: string | null = user?.company?.id || null;
      let company: any = null;

      if (companyId) {
        company = await apiService.getCompanyById(companyId);
      } else if (companyCode) {
        company = await apiService.getCompanyByCode(companyCode);
        companyId = company?.id || null;
      }

      if (company && companyId) {
        setResolvedCompanyId(companyId);
        // Prefill and lock company info when already exists
        setFormData(prev => ({
          ...prev,
          companyName: company.name || prev.companyName,
          companyCode: company.code || prev.companyCode,
      ...
      const [savedCountries, savedBranches] = await Promise.all([
          apiService.getCountries(companyId),
          apiService.getBranches(companyId)
      ]);
  ...
  }, [companyCode, user?.company?.id]);

  // Load saved data on component mount
  useEffect(() => {
    if (user?.company?.id || companyCode) {
      loadSavedData();
    }
  }, [user?.company?.id, companyCode, loadSavedData]);
  ...
  const addCountry = async () => {
    ...
    // Resolve company id prioritizing resolvedCompanyId
  ...
  const handleNext = () => {
    ...
    // Skip company info step if existing company is loaded
    if (existingCompanyLoaded && currentStep === 0) {
  ...
  const handleCompleteSetup = async () => {
    try {
      ...
      // Final guard: ensure required entities exist
      if (formData.countries.length === 0) {
      ...
      // If company already exists, update it; otherwise, create
      let companyId = resolvedCompanyId;
      if (!companyId && companyCode) {
        const existing = await apiService.getCompanyByCode(companyCode);
      ...
      // No need to recreate here. We only proceed to finish.
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <input
                  ...
                  disabled={existingCompanyLoaded}
                />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">
