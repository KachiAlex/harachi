...
const VendorsUpload: React.FC = () => {
  ...
  const handleImport = async () => {
    ...
    try {
      setLoading(true);
      setError(null);
      const storageRef = ref(storage, `uploads/${user.companyId}/vendors/${Date.now()}_${file.name}`);
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-bold text-gray-900">Upload Vendors</h1>
      <div className="card space-y-4">
        ...
        {successCount !== null && <div className="text-green-700">Imported {successCount} vendors</div>}
      </div>
    </div>
  );
};
