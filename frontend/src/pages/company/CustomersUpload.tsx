...
const CustomersUpload: React.FC = () => {
  const { user } = useAuth();
  ...
  const handleImport = async () => {
    ...
    try {
      setLoading(true);
      setError(null);
      const storageRef = ref(storage, `uploads/${user.companyId}/customers/${Date.now()}_${file.name}`);
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-bold text-gray-900">Upload Customers</h1>
      <div className="card space-y-4">
        ...
        {successCount !== null && <div className="text-green-700">Imported {successCount} customers</div>}
      </div>
    </div>
  );
};
